...
class Aircraft:
    __slots__ = (
    "id", "currCoordinate", "tmpTargetCoordinate", "targetCoordinates", "trackCoordinates", "_lock")
    ...
    def getTrackCoordinates(self):
        ...
        try:
            if len(self.trackCoordinates[0]) == 1:
                return self.trackCoordinates[0][0], self.trackCoordinates[1][0], self.trackCoordinates[2][0]
            if len(self.trackCoordinates[0]) > 1:
                return self.trackCoordinates[0].pop(0), self.trackCoordinates[1].pop(0), self.trackCoordinates[2].pop(0)
        finally:
            self._lock.release()
    ...
    def getTrackNumber(self):
        self._lock.acquire()
        try:
            return len(self.trackCoordinates[0])
        finally:
            self._lock.release()